...
from torch_geometric.utils import softmax, dense_to_sparse, add_remaining_self_loops, index_to_mask
...
from utils import plt_dictionary
...
class BernPool(torch.nn.Module):
    def __init__(self,in_channels,args, num_layer):
        ...
        # self.A=Parameter(torch.Tensor(self.in_channels,self.in_channels),requires_grad=True)
        # nn.init.orthogonal(self.A.data)

        # self.weight1 = nn.Parameter(torch.Tensor(in_channels, in_channels))
        # nn.init.xavier_uniform_(self.weight1.data)
        # self.weight2 = nn.Parameter(torch.Tensor(in_channels, in_channels))
        # nn.init.xavier_uniform_(self.weight2.data)
        # self.weight3 = nn.Parameter(torch.Tensor(in_channels, in_channels))
        # nn.init.xavier_uniform_(self.weight3.data)

        # self.attention_weight = nn.Parameter(torch.Tensor(in_channels, 1))
        # nn.init.xavier_uniform_(self.attention_weight)
        # dic_num = int(args.n_dic * (args.pool_ratio ** num_layer))
        # self.dic = Parameter(torch.Tensor(dic_num, self.in_channels),requires_grad=True)

        self.dic = Parameter(torch.Tensor(args.n_dic, self.in_channels),requires_grad=True)
        nn.init.orthogonal(self.dic.data)
        # self.dic = Parameter(torch.rand(args.n_dic, self.in_channels), requires_grad=True)
        c = np.array([1])
        self.c = nn.Parameter(torch.Tensor(c), requires_grad = True)

        # nn.init.xavier_uniform_(self.dic.data)
        # torch.nn.init.kaiming_normal_(self.dic, mode='fan_out', non_linearity='relu')

        # self.concat_linear = torch.nn.Linear(in_channels * 3, in_channels)
        # self.lin1 = MLP(in_channels = args.n_dic, hidden_channels = args.n_dic // 2, out_channels = 1, num_layers = 2)
        # self.lin1 = torch.nn.Linear(dic_num,1)
        self.lin1 = torch.nn.Linear(args.n_dic,1)
    ...
    # def similarity(self, K, Q, xlambda=9.0):
    #     # K:(N,d) Q:(M,d)
    #     N, d = K.size(0), K.size(1)
    #     Q = torch.transpose(Q, 0, 1).contiguous()
    #     attn = torch.mm(K, Q) #(N,M)
    #
    ...
    def dictionary_loss(self):
        # pdb.set_trace()
        I = torch.eye(self.dic.shape[0]).cuda()
        ortho_regularization = torch.matmul(self.dic, torch.transpose(self.dic, 0, 1)) - self.c * I
        dic_loss = torch.norm(ortho_regularization, p = "fro")
        return dic_loss

    # def filter_feature(self, x, edge_index, edge_attr, batch, bern_mask, epoch):
    #     # 若某些图的节点会被全部抛弃，则说明该图节点数已经较少，不再抛弃。
    ...
    #         # pdb.set_trace()
    #         A = to_dense_adj(edge_index, max_num_nodes=x.size(0), edge_attr=norm, batch = edge_index.new_zeros(x.size(0))).squeeze(0)
    #         sample_matrix_adj = A[mask_index]
    #         pool_x_adj = torch.matmul(sample_matrix_adj, x)
    #
    #     sample_matrix_diag = torch.diag(bern_mask)
    #     pool_x_diag = torch.mm(sample_matrix_diag[mask_index], x)
    #
    #     if edge_index.size(1) != 0:
    #         pool_x = pool_x_diag + pool_x_adj
    #     else:
    #         pool_x = pool_x_diag
    ...
    def filter_feature(self, x, edge_index, edge_attr, batch, bern_mask, epoch):
        ...
        sample_matrix_adj = A[mask_index]
        pool_x = torch.matmul(sample_matrix_adj, x)

        # sample_matrix_diag = torch.diag(bern_mask)
        # pool_x = torch.mm(sample_matrix_diag[mask_index], x)
    ...
    def dropout_node(self, edge_index, p=0.8,num_nodes = None,training: bool = True):
    ...
    def random_dropout_node(self,x, edge_index, edge_attr, batch):
        edge_index, edge_mask, node_mask = self.dropout_node(edge_index, num_nodes=x.shape[0])
        index = self.mask_to_index(node_mask)
        pool_x = x[index]
        pool_batch = batch[index]
        pool_edge_index, pool_edge_attr = filter_adj(edge_index, edge_attr, index, num_nodes=x.size(0))
        # pdb.set_trace()
        return pool_x, pool_edge_index, pool_edge_attr, pool_batch

    def forward(self,x, edge_index, edge_attr, batch, epoch):
        # dropnode
        # pool_x, pool_edge_index, pool_edge_attr, pool_batch = self.random_dropout_node(x, edge_index, edge_attr, batch)
        # return pool_x, pool_edge_index, pool_edge_attr, pool_batch

        #(number of Nodes, number of dictionary)

        similarity = self.cosine_similarity(x, self.dic)
        # 融合字典特征
        score = self.lin1(similarity).squeeze()
        score = torch.sigmoid(score)
        bern_mask = torch.bernoulli(score)
        # index = self.mask_to_index(bern_mask)

        pool_x, pool_edge_index, pool_edge_attr, pool_batch, mask_index = self.filter_feature(x, edge_index, edge_attr, batch, bern_mask, epoch)
        # 添加KL散度
        kl_loss = self.KL_divgence(score.mean())
        dic_loss= self.dictionary_loss()
        # pdb.set_trace()
        # sparse_loss = torch.mean(torch.abs(score - self.ratio))
        # sparse_loss = torch.sqrt(torch.mean((score - self.ratio)**2)) * (-1)
        return pool_x, pool_edge_index, pool_edge_attr, pool_batch, kl_loss, dic_loss
