...
class GraphClassificationModel(torch.nn.Module):
    ...
    def forward(self, data, hard):

        x, edge_index, batch = data.x, data.edge_index, data.batch
        edge_attr = None

        x = F.relu(self.conv1(x, edge_index, edge_attr))
        x, edge_index, edge_attr, batch, kl_loss1, ref_loss1 = self.bern_pool1(x, edge_index, edge_attr, batch, hard)
        x1 = torch.cat([gmp(x, batch), gap(x, batch)], dim=1)
        
        x = F.relu(self.conv2(x, edge_index, edge_attr))
        x, edge_index, edge_attr, batch, kl_loss2, ref_loss2 = self.bern_pool2(x, edge_index, edge_attr, batch, hard)
        x2 = torch.cat([gmp(x, batch), gap(x, batch)], dim=1)
        
        x = F.relu(self.conv3(x, edge_index, edge_attr))
        x, edge_index, edge_attr, batch, kl_loss3, ref_loss3 = self.bern_pool3(x, edge_index, edge_attr, batch, hard)
        ...
        #return x,kl_loss1+ kl_loss2,ref_loss1+ ref_loss2
