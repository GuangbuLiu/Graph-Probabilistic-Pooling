...
class BernPool(torch.nn.Module):
    ...
    def forward(self,x, edge_index, edge_attr, batch, hard):

        similarity = self.cosine_similarity(x, self.ref)
        score = self.lin1(similarity).squeeze()
        score = torch.sigmoid(score)
        score_two=torch.stack((score,1-score),dim=1)
        #pdb.set_trace()
        bern_mask=gumbel_softmax(score_two, temperature=0.5, hard=hard)[:,0]
        #pdb.set_trace()
